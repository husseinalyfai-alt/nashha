...
for source, url in FEEDS:
    try:
        ...
        for entry in feed.entries[:30]:
            ...
            items.append(make_item(title, source, link, entry.get("published", ""), summary)); seen.add(link)
...
if bearer:
    try:
        ...
        for post in r.json().get("data", []):
            ...
            items.append(make_item(text[:160], "X", link, post.get("created_at", ""), text, "X")); seen.add(link)
...
if fb_page_id and fb_token:
    try:
        ...
        for post in r.json().get("data", []):
            ...
            items.append(make_item(text[:160], "Facebook", link, post.get("created_time", ""), text, "Facebook")); seen.add(link)
